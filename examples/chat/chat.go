...
func init() {
	RegistCommand(NewCommand("broadcast",func (cln *client.Client,args ...string) string {
		if len(args)!=0 {
			m :="MESSAGE:" + fmt.Sprint(cln.Id)+" "+args[0]
	...
	RegistCommand(NewCommand("say_to",func (cln *client.Client,args ...string) string {
		if len(args)!=3 {
		...
		m := "MESSAGE:" + fmt.Sprint(cln.Id) + " " + args[2]
		...
		switch args[0] {
		case "name":
			rc= cln.ServerContext.FindClientByName(args[1])
		case "id":
			id,err :=strconv.ParseInt(args[1],0,64)
			if err!=nil {
				return "ERROR:The id subcommand  need a integer64"
			} else {
				rc= cln.ServerContext.FindClientById(id)
			}
		}
